...
const ScreenList = ({
    ...
    return (
        <aside className={`
            ...
            w-[85%] md:w-96 h-full border-r border-slate-200 p-8 flex flex-col gap-8 bg-slate-50/95 md:bg-slate-50/50 backdrop-blur-3xl 
        ...
        `}>
            <div className="hidden md:flex items-center gap-3 px-2">
                ...
                <div>
                    <h1 className="font-black text-xl tracking-tight leading-none text-zinc-900">Totem Cloud</h1>
            ...
            <div className="flex-1 overflow-y-auto pr-2 custom-scrollbar space-y-6">
                ...
                <AnimatePresence>
                    {isAddingScreen && (
                        ...
                        onSubmit={handleAddScreen} className="bg-white shadow-sm p-4 rounded-2xl border border-orange-500/20 overflow-hidden"
                        >
                            <div className="space-y-3">
                                ...
                                className="w-full bg-white shadow-sm border border-slate-300 rounded-xl px-3 py-2 text-sm outline-none focus:border-orange-500 transition-all font-bold" required />
                                ...
                                <div className="p-3 bg-slate-100 border border-slate-200 rounded-xl space-y-2">
                                    ...
                                    <div className="flex gap-2">
                                        ...
                                        className="flex-1 bg-white shadow-sm border border-slate-300 rounded-lg px-3 py-2 text-sm outline-none focus:border-orange-500 transition-all font-bold" />
                                    ...
                                    {(newScreenData.lat && newScreenData.lon) && (
                                        <div className="flex gap-2 pt-1 opacity-70">
                                            <div className="flex-1 bg-white shadow-sm rounded-lg px-2 py-1.5 text-xs text-emerald-400 font-mono truncate shadow-inner">Lat: {newScreenData.lat}</div>
                                            <div className="flex-1 bg-white shadow-sm rounded-lg px-2 py-1.5 text-xs text-emerald-400 font-mono truncate shadow-inner">Lon: {newScreenData.lon}</div>
                                        </div>
                                    )}
                                </div>
                                ...
                                <button type="button" onClick={() => setIsAddingScreen(false)} className="px-3 py-2 bg-slate-100 hover:bg-white/20 text-zinc-500 rounded-xl text-[10px] font-black uppercase transition-all">X</button>
                                </div>
                            </div>
                        </motion.form>
                    )}
                    ...
                    {screens.map(screen => {
                        ...
                        return (
                            <button key={screen.id} onClick={() => handleSelectScreen(screen)}
                                className={`w-full flex flex-col gap-2 items-start justify-between px-4 py-3 rounded-2xl transition-all group ${selectedScreen?.id === screen.id ? 'bg-orange-500 text-white shadow-lg shadow-orange-500/20' : 'text-zinc-500 hover:bg-white shadow-sm'}`}
                            >
                                ...
                                <div className="w-full flex items-center gap-2 px-1">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-[11px] font-bold truncate text-zinc-800" title={nowPlayingItem?.url || nowPlayingLabel}>{nowPlayingItem ? nowPlayingItem.url : nowPlayingLabel}</p>
                                    </div>
                                </div>
                            </button>
                        );
